...
closeBtn.addEventListener("click", () => {
    if (mobileNav.classList.contains(navClosedClass)) {
        // Remove the closed class and add the opened class
        mobileNav.classList.remove(navClosedClass);
        mobileNav.classList.add(navOpenedClass);
    } else {
        // Remove the opened class and add the closed class
...
document.querySelectorAll('.nav-link').forEach(link => {
    link.addEventListener('click', function(e) {
      ...
      // Calculate the top offset position of the target element
      const targetPosition = targetElement.getBoundingClientRect().top + window.pageYOffset;
  
      // Smooth scroll to the target position with a duration of 400ms
      window.scrollTo({
        top: targetPosition,
        behavior: 'smooth'
      });
  
      // You can manually adjust the scroll duration with setTimeout
      setTimeout(() => {
        window.scrollTo({
          top: targetPosition,
          behavior: 'auto' // Jump to the target to ensure it's exactly at the position
        });
      }, 2400);
    });
  });
